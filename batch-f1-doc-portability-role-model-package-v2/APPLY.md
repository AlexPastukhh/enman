# Apply Batch F1 Package v2

Run from repo root. This corrected v2 package should replace the first F1 package output before commit.

```powershell
git checkout my-changes
git pull --ff-only origin my-changes

Copy-Item -Path "$env:USERPROFILE\Downloads\batch-f1-doc-portability-role-model-package-v2.zip" -Destination ".\batch-f1-doc-portability-role-model-package-v2.zip" -Force

Expand-Archive -Path ".\batch-f1-doc-portability-role-model-package-v2.zip" -DestinationPath ".\" -Force

$pkg = "batch-f1-doc-portability-role-model-package-v2"
...
$pkgName = "batch-f1-doc-portability-role-model-package-v2"
