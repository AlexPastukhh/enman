...
- create individual applicant party only in integration tests when persisted `ClientAccount.Id` is available;
- cannot create without full name;
- cannot create without email/phone.

### ClientRequest

- create connection request only in integration tests when persisted `ApplicantParty.Id` is available;
...
- Domain unit tests should not depend on EF-generated IDs.
- When a factory requires an existing referenced aggregate and that referenced aggregate ID is generated by EF, full ID propagation is an integration-test concern.
- Domain unit tests must not imitate a persisted aggregate by setting `Entity.Id` through reflection, test hooks, fake EF behavior or other manual ID mutation.
- If successful creation depends on an already persisted referenced aggregate, cover that successful path in integration tests, not unit tests.
- Unit tests should cover:
  - local aggregate invariants;
  - guard/exception behavior for null required domain objects/value objects;
  - guard/exception behavior when a referenced aggregate is transient with default `Id <= 0`, only when this can be checked without imitating persistence.
- Integration tests should cover:
  - create `ClientAccount`;
  - save and obtain generated `ClientAccount.Id`;
  - create `IndividualApplicantParty` from persisted `ClientAccount`;
  - save and verify `ClientAccountId`;
  - create `ConnectionRequest` from persisted `IndividualApplicantParty`;
  - save and verify `ApplicantPartyId`.
