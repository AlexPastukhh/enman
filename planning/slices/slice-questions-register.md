...
| SL-REQ-Q-001 | `SL-REQ-001-create-connection-request.md` / `SL-REQ-002-my-requests-list.md` / `SL-REQ-003-own-request-details.md` | My Requests read model | resolved for first server read cuts | What exact My Requests list/detail response does the client need after request success? | List summary is implemented in `SL-REQ-002`; own details are implemented in `SL-REQ-003`; client UI remains future work. | future client sidecar | no for server read |
...
| SL-REQ-Q-013 | `SL-REQ-003-own-request-details.md` | Applicant snapshot | future review | Should own request details include applicant snapshot? | No in first cut; add deliberately when UI/source behavior needs it. | details API/client contract | no |
| SL-REQ-Q-014 | `SL-REQ-003-own-request-details.md` | Feedback action | future review | Should backend return a `canCreateNewFromFeedback` flag? | No in first cut; client can derive action from `status === Rejected` until policy becomes non-trivial. | details client behavior | no |
| SL-REQ-Q-015 | `SL-REQ-003-own-request-details.md` | Agreement/documents | future review | Should approved details include agreement/documents data? | No; future agreement/documents slices own that payload. | future agreement/documents slices | no |
| SL-REQ-Q-016 | `SL-REQ-003-own-request-details.md` | Missing/not-owned | resolved | Should not-owned request details reveal existence? | No. Missing and not-owned both return 404. | API security/tests | no |
| SL-REQ-Q-017 | `SL-REQ-003-own-request-details.md` | Rejection feedback | resolved | How should rejected feedback be represented? | `reviewResult.rejection.reason`, mapped from persisted `RejectionFeedback.Value` / review rejection reason. | details API/client contract | no |
