...
| SL-APPL-CLIENT-Q-001 | `SL-APPL-001-create-individual-applicant-party.client.md` | Current applicant read | open | How does Account page load an existing applicant after refresh? | Current client create flow uses local post-submit state. Stable refresh requires future current applicant read endpoint/client slice. | Account page stability, verification status, request UI readiness | yes before claiming full persisted Account page state |
