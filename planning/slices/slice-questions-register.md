...
Status: active register / synchronized with current implemented backend L1 slice docs and current client-missing state  
...
| SL-REQ-Q-003 | `SL-REQ-001-create-connection-request.md` | Client UI | open | What concrete page/form implements request creation? | Draft assumes no `.client.md` is created until concrete client work starts. | request creation client sidecar | no for backend docs; yes before request UI implementation |
