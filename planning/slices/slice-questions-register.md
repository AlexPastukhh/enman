...
Status: active register / synchronized with current implemented backend L1 slice docs, current client-missing state and Applicant Data UI sidecar draft  
...
| SL-AUTH-Q-001 | `SL-AUTH-001-login-client-account.md` | Client auth baseline | open | What concrete client state/store/query shape owns L1 current-user after login? | Draft assumes this belongs to the first auth/session `.client.md` sidecar. | auth/session client baseline | yes before client auth implementation |
| SL-AUTH-Q-002 | `SL-AUTH-001-login-client-account.md` / `SL-ACC-001-register-client-account.md` | Auth UX | open | Should registration automatically login or require explicit login? | Current backend register does not issue a session; login endpoint does. Draft assumes explicit login unless user confirms auto-login flow. | registration/login client flow | yes before final registration UX |
| SL-AUTH-Q-003 | `SL-AUTH-001-login-client-account.md` | Auth hardening | future review | When should lockout/rate limiting be introduced? | Not part of current implemented backend slice. | auth/security hardening | no |
| SL-AUTH-Q-004 | `SL-AUTH-002-current-user.md` | Client bootstrap | open | Should client call current-user on app bootstrap, protected route entry, or both? | Draft assumes this belongs to auth/session `.client.md`. | auth provider / route guard / query cache | yes before client auth baseline |
| SL-AUTH-Q-005 | `SL-AUTH-002-current-user.md` | Error UX | open | How should client distinguish unauthenticated from server failure during current-user bootstrap? | Draft assumes Unauthorized clears/keeps guest state; 500/global failure needs separate UI handling. | client error handling and route UX | yes before client auth baseline |
| SL-AUTH-Q-006 | `SL-AUTH-003-logout.md` | Client logout UX | open | Where should the client navigate after logout? | Draft assumes auth/session `.client.md` will define the visible post-logout route. | logout UI/navigation | yes before client auth baseline |
| SL-AUTH-Q-007 | `SL-AUTH-003-logout.md` | CSRF/security | open | When should logout require antiforgery token handling? | Draft assumes CC-CSRF-001 governs concrete unsafe-browser command handling. Current logout backend slice does not implement CSRF. | CSRF cross-cutting slice and logout client work | no for backend docs; yes before claiming CSRF implementation |
| SL-AUTH-Q-008 | `SL-AUTH-003-logout.md` | Client cache | open | Should logout clear only auth state or also invalidate all user-scoped query caches? | Draft assumes auth/session sidecar owns cache invalidation decision. | client auth/cache design | yes before client auth baseline |
...
| SL-APPL-CLIENT-Q-004 | `SL-APPL-001-create-individual-applicant-party.client.md` | Account page read state | open | How should Account page know whether current applicant data already exists? | Draft assumes a separate `L1-APPLICANT-PARTY-READ-CURRENT` slice with server-derived current account context and target endpoint direction `GET /api/l1/applicant-parties/current-individual`. Initial create UI may switch to local read-only state after success. | future read-current slice; Account page stable refresh state | no for create sidecar draft; yes before final stable Account page implementation |
| SL-APPL-CLIENT-Q-006 | `SL-APPL-001-create-individual-applicant-party.client.md` | Session state | accepted direction | Should applicant party creation refetch current-user/session? | No. Current-user/session is account authentication state. Applicant party state belongs to applicant party read model unless current-user contract explicitly changes later. | auth/session client baseline; Account page state model | no |
| SL-APPL-CLIENT-Q-007 | `SL-APPL-001-create-individual-applicant-party.client.md` | Verification UI | future review | Should Account page show applicant party verification status? | Future Account page should show verification status when the read model exposes it. Initial create UI only shows saved applicant data as read-only. | `L1-APPLICANT-PARTY-READ-CURRENT`; verification flow | no |
| SL-APPL-CLIENT-Q-008 | `SL-APPL-001-create-individual-applicant-party.client.md` | Request entry location | accepted direction | Should Account page show create request entry before applicant party data exists? | No. This sidecar does not introduce create request entry. Future `L1-CONNECTION-REQUEST-CREATE.client` decides where the create request entry belongs. | request creation client sidecar; global navigation | no for applicant create sidecar |
...
| SL-REQ-Q-003 | `SL-REQ-001-create-connection-request.md` | Client UI | open | What concrete page/form implements request creation? | Draft assumes no request `.client.md` is created until concrete request client work starts. Applicant create sidecar does not introduce the request entry. | request creation client sidecar | no for backend docs; yes before request UI implementation |
