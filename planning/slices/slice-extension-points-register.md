...
| CP-REQ-DETAILS-APPLICANT-001 | SL-REQ-003 | Server + Client | Applicant snapshot in request details | details/read contract | no applicant snapshot in first cut | no | details API/client tests later | future review |
| CP-REQ-DETAILS-FEEDBACK-001 | SL-REQ-003 | Server + Client | Create-new-from-feedback action | details/client action | no server flag now; client can derive simple action from Rejected status | no | details client tests later | future review |
| CP-REQ-DETAILS-DOCS-001 | SL-REQ-003 | Server + Client | Agreement/documents payload | details/read contract | agreement/documents are not included in first cut | no | future agreement/documents tests | future review |
