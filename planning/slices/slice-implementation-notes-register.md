...
Status: active / applicant-template-per-type synchronized / server validation notes added
...
| `NOTE-VALIDATION-001` | `CC-VALIDATION-001` | server/API | L1 request DTO FluentValidation is not a consistent implemented layer yet; future changed L1 API input should plan it explicitly. | accepted direction |
| `NOTE-VALIDATION-002` | `SL-REQ-001` | server/API | Target Existing/New applicant context DTO needs request-level validation for discriminator, required branch payload and mutually exclusive fields. | accepted direction |
| `NOTE-VALIDATION-003` | `CC-VALIDATION-001` | testing | E2E should assert user-visible validation outcomes; validator/unit and API integration tests own FluentValidation mechanics. | accepted direction |
