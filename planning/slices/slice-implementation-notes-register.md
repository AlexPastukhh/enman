...
| `NOTE-APPL-SVC-001` | `SL-APPL-001` / `SL-APPL-004` / `SL-REQ-001` | application | Shared creation service exists, has no SaveChanges, standalone handler commits, and request New branch reuses it. | implemented |
| `NOTE-REQ-APPL-001` | `SL-REQ-001` | API | Request create uses explicit Existing/New applicant context. | implemented |
| `NOTE-REQ-APPL-002` | `SL-REQ-001` | transaction | New applicant + request are created through one request handler SaveChanges. | implemented |
