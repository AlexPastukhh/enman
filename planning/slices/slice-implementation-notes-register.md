...
|---|---|---|---|---|---|---|---|---|
| NOTE-APPL-READ-001 | `L1-APPLICANT-PARTY-READ-CURRENT` | SC-10 | Server/API/Client | read-current, account-page, normal-empty-state | Current applicant read returns normal Account page state when missing: `200 exists=false`, not exception-like `404`. | Keeps Account page able to show create form as normal empty state. | implemented slice + slice questions register | resolved |
...
| NOTE-APPL-CLIENT-002 | `SL-APPL-001.client` / `L1-APPLICANT-PARTY-READ-CURRENT.client` | SC-10 | Client/UI | read-current, refresh | Current applicant UI uses server-backed read state after refresh. Create success invalidates/refetches current applicant query. | Required before claiming full persisted Account page applicant state. | implemented `L1-APPLICANT-PARTY-READ-CURRENT` | resolved |
