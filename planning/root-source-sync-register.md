...
| `planning/planning-use-case-map.md` | covered active core | Doc version: v0.6.0 | root action/router coverage done; source/version maintenance and current-state command routes added in SRC-CMD-1A; current source labels refreshed in SRC-CMD-1C | Refresh routes as commands change. |
...
| `planning/documentation/review-diff-file-workflow.md` | explicit repo-stored review diff workflow | `planning/replacement-file-generation-guide.md`; `planning/planning-use-case-map.md`; `_ai-review-diffs/last-archive.diff` when explicit mode is used | output/process + review artifact | review-diff workflow Doc version: v0.1.0; replacement guide Doc version: v0.1.0; use-case map Doc version: v0.6.0 | derived/synchronized for ROOT-SRC-3A local Sources scope; current router label refreshed in SRC-CMD-1C | local Sources added; explicit-only boundary preserved | Used only when user explicitly requests repo-stored review diff. |
