...
| `tools/tampermonkey/chat-command-palette.user.js` | userscript implementation | implementation/helper source, version not applicable; userscript @version 0.3.0 | derived/synchronized in ROOT-FULL-1 and updated in SRC-CMD-1B narrow projection consistency pass | Last in source-of-truth order; comment added for source sync boundary; must not invent command semantics or receive a Doc version without a code version policy. |
...
| `planning/planning-use-case-map.md` | covered active core | Doc version: v1.0.0 | root action/router coverage done; source/version maintenance and current-state command routes added in SRC-CMD-1A; current source labels refreshed in SRC-CMD-1C | Refresh routes as commands change. |
...
| `planning/workstreams/tampermonkey-command-projection-plan.md` | ROOT-FULL-1/SRC-CMD-1B-covered helper projection plan | Doc version: v0.3.0 | helper profiles added for source/version maintenance commands and `положняк` in SRC-CMD-1B | Refresh when command profiles/routes change. |
...
| `tools/tampermonkey/README.md` | ROOT-FULL-1/SRC-CMD-1B-covered helper implementation docs | Doc version: v0.3.0 | helper profile docs updated in SRC-CMD-1B | Not command source of truth. |
...
| `tools/tampermonkey/chat-command-palette.user.js` | implementation/helper | implementation/helper source, version not applicable; userscript @version 0.3.0 | source sync boundary comment kept; helper profiles added in SRC-CMD-1B | Projection consistency checked; do not introduce Doc version without code version policy. |
