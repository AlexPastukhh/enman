...
Status: skeleton / root planning dependency register partially derived for source-governance core, root router/onboarding core, protocol/role core, output/archive core and Goal Map/Tampermonkey active files, source/version maintenance commands, current-state template, Tampermonkey helper source/version command profiles, stale source-label cleanup and targeted source-impact cleanup, root route/example-fit cleanup and slice/testing source-sync register skeletons, slice/testing owner workflow/template version seed and slice/test refactor Goal Map sync and slice/test audit-card findings; root folder inventory classified, command-aware Workflow Preflight hardening, English command/use-case name projection and reusable Tampermonkey projection/tool starter extraction and legacy current-like root cleanup
Doc version: v2.3.0
...
After CASCADE-CMD-PREFLIGHT-0, this register records command-aware Workflow Preflight hardening and Tampermonkey command-body guardrail updates. These are workflow/helper projection changes, not a new command source-of-truth model.

After CMD-TM-EN-NAMES-1, this register records neutral English command/use-case display names in UCM and Tampermonkey helper projections. English names are display labels only, not separate command authority.

After ROOT-LEGACY-CLEAN, this register records removal of stale current-like/root bridge files from active routing and collapses their former inventory rows into a deleted legacy cleanup note.
...
| `planning/workstreams/command-system-and-tampermonkey-goal-map.md` | living command-system/Tampermonkey Goal Map | Doc version: v0.3.0 | derived/synchronized in ROOT-FULL-1 narrow Goal Map/Tampermonkey source pass; helper profile state refreshed in SRC-CMD-1B | Workstream state artifact; not a generic workflow; local source sync section added; snapshot remains SL-6 smoke testing. |
| `planning/workstreams/tampermonkey-command-projection-plan.md` | Tampermonkey command projection plan | Doc version: v0.5.0 | derived/synchronized in ROOT-FULL-1 and updated in SRC-CMD-1B helper profile pass | Defines helper prompt body projection model; local source sync section added; helper profiles remain projections. |
...
| `tools/tampermonkey/README.md` | Tampermonkey helper orientation | Doc version: v0.5.0 / implementation docs | derived/synchronized in ROOT-FULL-1 and updated in SRC-CMD-1B helper profile pass | Not command source of truth; local source sync section added. |
...
| `tools/tampermonkey/chat-command-palette.user.js` | userscript implementation | implementation/helper source, version not applicable; userscript @version 0.4.0 | derived/synchronized in ROOT-FULL-1 and updated in SRC-CMD-1B narrow projection consistency pass | Last in source-of-truth order; comment added for source sync boundary; must not invent command semantics or receive a Doc version without a code version policy. |
...
|---|---|---|---|---|
| `planning/README.md` | covered active core | Doc version: v0.4.0 | root onboarding/navigation coverage done | Refresh only when dependencies change. |
| `planning/planning-use-case-map.md` | covered active core | Doc version: v1.1.0 | root action/router coverage done; source/version maintenance and current-state command routes added in SRC-CMD-1A; current source labels refreshed in SRC-CMD-1C | Refresh routes as commands change. |
| `planning/workflow-activation-map.md` | covered active core | Doc version: v0.6.0 | workflow activation coverage done | Refresh after new workflow owners are added. |
| `planning/planning-doc-responsibility-map.md` | covered active core | Doc version: v0.6.0 | root responsibility coverage done | Refresh after new responsibility owners are added. |
...
| `planning/workstreams/tampermonkey-command-projection-plan.md` | ROOT-FULL-1/SRC-CMD-1B-covered helper projection plan | Doc version: v0.5.0 | helper profiles added for source/version maintenance commands and `положняк` in SRC-CMD-1B | Refresh when command profiles/routes change. |
| `planning/deferred-goals-and-ideas.md` | ROOT-FULL-1-covered active backlog | Doc version: v0.3.0 | local source sync section added in ROOT-FULL-1 | Future stale-version/local-source audit commands parked here. |
...
| `tools/tampermonkey/README.md` | ROOT-FULL-1/SRC-CMD-1B-covered helper implementation docs | Doc version: v0.5.0 | helper profile docs updated in SRC-CMD-1B | Not command source of truth. |
...
| `tools/tampermonkey/chat-command-palette.user.js` | implementation/helper | implementation/helper source, version not applicable; userscript @version 0.4.0 | source sync boundary comment kept; helper profiles added in SRC-CMD-1B | Projection consistency checked; do not introduce Doc version without code version policy. |

### Removed legacy/current-like root files — ROOT-LEGACY-CLEAN

| Removed group | Former examples | Current routing decision |
|---|---|---|
| Current-like root notes | `planning/current-state.md`, `planning/planning-workflow-current.md`, `planning/l1-current-implementation-status.md`, `planning/l2-current-planning-status.md` | Removed from active routing. Use root UCM, Workflow Activation Map, living Goal Maps, source/register files and current repo evidence instead. |
| Old agent/root rules | `planning/agent-rules.md`, old root bridge/status notes | Removed or no longer active. Use `planning/planning-agent-protocol.md`, `planning/agent-roles-and-required-actions.md` and responsibility maps. |
| Legacy bridge/diagram/cleanup notes | old diagram root files, cleanup/replacement notes and superseded bridge files | Removed from active inventory or kept only in external history. Do not route new chats to these paths. |
...
Legacy diagram root bridge files removed by ROOT-LEGACY-CLEAN are not active inventory rows. Current diagramming source ownership should be checked in the diagram/diagramming layer before reintroducing root-level rows.
...
| `planning/documentation/review-diff-file-workflow.md` | explicit repo-stored review diff workflow | `planning/replacement-file-generation-guide.md`; `planning/planning-use-case-map.md`; `_ai-review-diffs/last-archive.diff` when explicit mode is used | output/process + review artifact | review-diff workflow Doc version: v0.1.0; replacement guide Doc version: v0.1.0; use-case map Doc version: v1.0.0 | derived/synchronized for ROOT-SRC-3A local Sources scope; current router label refreshed in SRC-CMD-1C; route/read dependency classification preserved in CASCADE-SRC-1A | local Sources added; explicit-only boundary preserved | Used only when user explicitly requests repo-stored review diff. |
...
| `planning/goal-map-principles-workflow-template.md` | Goal Map rules, brief shape and map sync command | `planning/planning-use-case-map.md`; `planning/documentation/reviewable-agent-output-and-commands-workflow.md`; `planning/goal-map-example.md`; `planning/documentation/examples/GOAL-MAP-BRIEF-RESPONSE-EXAMPLE.md`; `planning/documentation/examples/GOAL-MAP-SYNC-COMMAND-EXAMPLE.md`; relevant living Goal Map files | workflow/template + response-shape sources | Goal Map owner Doc version: v0.1.0; use-case map Doc version: v1.0.0; reviewable workflow Doc version: v0.1.0; goal-map-example Doc version: v0.2.0; example versions mixed/not fully confirmed | derived/synchronized for ROOT-FULL-1 narrow Goal Map/Tampermonkey source pass; current router label refreshed in SRC-CMD-1C; route/read dependency classification preserved in CASCADE-SRC-1A | local/file-level source sync section added in ROOT-FULL-1 | Owns living Goal Map rules, brief shape and `синх карта`; durable state lives in living Goal Maps, not in brief output. |
| `planning/workstreams/command-system-and-tampermonkey-goal-map.md` | living state for command-system/Tampermonkey workstream | `planning/goal-map-principles-workflow-template.md`; `planning/planning-use-case-map.md`; `planning/documentation/examples/README.md`; `planning/workstreams/tampermonkey-command-projection-plan.md`; `tools/tampermonkey/README.md`; `tools/tampermonkey/chat-command-palette.user.js`; `planning/replacement-file-generation-guide.md`; `planning/documentation/review-diff-file-workflow.md` | living-state/workstream source + route/process references | command-system workstream Doc version: v0.3.0; Goal Map owner Doc version: v0.1.0; use-case map Doc version: v1.1.0; projection plan Doc version: v0.5.0; helper README Doc version: v0.5.0; userscript @version 0.4.0; replacement/review-diff docs Doc version: v0.1.0 | derived/synchronized in ROOT-FULL-1; helper profile state refreshed in SRC-CMD-1B; current router label refreshed in SRC-CMD-1C; route/read dependency classification preserved in CASCADE-SRC-1A | local/file-level source sync section added | Snapshot remains SL-6 helper smoke testing; userscript is projection, not command source of truth. |
| `planning/workstreams/tampermonkey-command-projection-plan.md` | helper command projection and prompt envelope | `planning/planning-use-case-map.md`; `planning/replacement-file-generation-guide.md`; `planning/documentation/review-diff-file-workflow.md`; `planning/goal-map-principles-workflow-template.md`; `planning/documentation/examples/README.md`; `tools/tampermonkey/README.md`; `tools/tampermonkey/chat-command-palette.user.js` | projection/process + command-route references | projection plan Doc version: v0.5.0; use-case map Doc version: v1.1.0; replacement/review-diff docs Doc version: v0.1.0; Goal Map owner Doc version: v0.1.0; helper README Doc version: v0.5.0; userscript @version 0.4.0 | derived/synchronized in ROOT-FULL-1 and updated in SRC-CMD-1B; current router label refreshed in SRC-CMD-1C; route/read dependency classification preserved in CASCADE-SRC-1A | local/file-level source sync section added; helper profiles are projections | Helper profiles are route hints; UCM and owner workflows remain authoritative. |
| `tools/tampermonkey/README.md` | helper orientation and inserted-command interpretation | `planning/workstreams/tampermonkey-command-projection-plan.md`; `planning/planning-use-case-map.md`; `tools/tampermonkey/IMPLEMENTATION-NOTES.md`; `tools/tampermonkey/chat-command-palette.user.js` | implementation orientation + projection reference | helper README Doc version: v0.5.0; projection plan Doc version: v0.5.0; use-case map Doc version: v1.1.0; implementation notes Doc version: v0.2.0; userscript @version 0.4.0 | derived/synchronized in ROOT-FULL-1 and updated in SRC-CMD-1B; current router label refreshed in SRC-CMD-1C; route/read dependency classification preserved in CASCADE-SRC-1A | local/file-level source sync section added; helper docs are not command source of truth | Not command source of truth. |
| `tools/tampermonkey/chat-command-palette.user.js` | userscript command profile implementation | `planning/workstreams/tampermonkey-command-projection-plan.md`; `planning/planning-use-case-map.md`; `planning/replacement-file-generation-guide.md`; `planning/documentation/review-diff-file-workflow.md`; `planning/goal-map-principles-workflow-template.md` | implementation projection source | userscript @version 0.4.0; projection plan Doc version: v0.5.0; use-case map Doc version: v1.1.0; replacement/review-diff docs Doc version: v0.1.0; Goal Map owner Doc version: v0.1.0 | derived/synchronized in ROOT-FULL-1 and updated in SRC-CMD-1B; current router label refreshed in SRC-CMD-1C; route/read dependency classification preserved in CASCADE-SRC-1A | helper profile implementation checked with node syntax only; command semantics remain in docs | Current command profiles should remain projections of route/owner docs. |
| `planning/documentation/planning-docs-architecture-principles.md` | reusable docs architecture principles: example reuse levels and filename/version/source boundaries | `planning/documentation/examples/README.md`; `planning/documentation/example-coverage-workflow.md`; `planning/documentation/field-kits/root-use-case-map-field-kit.md`; `planning/documentation/use-case-map-workflow.md` | reusable principle + example taxonomy | principles Doc version: v0.1.0; examples README Doc version: v0.2.0; field-kit/workflow/example-coverage files Doc version: v0.1.0 | skeleton / CASCADE-ROUTE-1B limited file-level sync | no broad source coverage claim | Added profile-specific reusable example taxonomy; examples remain demonstration-only. |
| `planning/documentation/examples/README.md` | examples index and example reuse-level navigation | `planning/documentation/planning-docs-architecture-principles.md`; `planning/documentation/example-coverage-workflow.md`; `planning/documentation/field-kits/root-use-case-map-field-kit.md`; `planning/documentation/use-case-map-workflow.md` | navigation/index + example taxonomy | examples README Doc version: v0.2.0; sources Doc version: v0.1.0 where declared | skeleton / CASCADE-ROUTE-1B limited file-level sync | example fit must be user/project accepted | Index distinguishes fully reusable, profile-specific reusable and project-local examples. |
| `planning/documentation/field-kits/root-use-case-map-field-kit.md` | root UCM setup and profile-specific example-fit confirmation | `planning/documentation/examples/README.md`; `planning/documentation/use-case-map-workflow.md`; `planning/documentation/example-coverage-workflow.md`; `planning/planning-use-case-map.md` | setup/process + demonstration-fit rule | field kit Doc version: v0.2.0; examples README Doc version: v0.2.0; use-case workflow/example-coverage Doc version: v0.1.0; UCM Doc version: v1.1.0; Tampermonkey projection workflow Doc version: v0.1.0 | skeleton / CASCADE-ROUTE-1B limited file-level sync | no silent example-fit decision | Field kit must show candidate profile-specific examples to the user before wiring them into concrete root UCM routes. |
| `planning/documentation/command-creation-workflow.md` | reusable command creation and Tampermonkey decision gate | `planning/planning-use-case-map.md`; `planning/documentation/reviewable-agent-output-and-commands-workflow.md`; `planning/documentation/use-case-map-workflow.md`; `planning/documentation/USE-CASE-MAP-TEMPLATE.md`; `planning/documentation/example-coverage-workflow.md`; `planning/documentation/examples/README.md`; `planning/documentation/tampermonkey-command-projection-workflow.md` | workflow/process + projection decision gate | command creation workflow Doc version: v0.2.0; projection workflow Doc version: v0.1.0; UCM Doc version: v1.1.0 | skeleton / REUSE-TM-1 reusable projection extraction | reusable authority moved away from Enman workstream/tools | Command creation remains rule-based; Tampermonkey is optional projection only. |
| `planning/documentation/tampermonkey-command-projection-workflow.md` | reusable Tampermonkey projection rules | project root UCM; command owner workflows/templates; project-local tools/tampermonkey files when adapted | reusable projection workflow | projection workflow Doc version: v0.1.0 | skeleton / REUSE-TM-1 new reusable owner | new reusable file added | Owns projection-only helper rules; project-local tools/workstreams do not own reusable command meaning. |
| `planning/documentation/tools/tampermonkey/README.md` | reusable Tampermonkey tool starter notes | `planning/documentation/tampermonkey-command-projection-workflow.md`; `planning/documentation/command-creation-workflow.md`; `planning/documentation/field-kits/root-use-case-map-field-kit.md` | reusable starter notes | tool starter README Doc version: v0.1.0; projection workflow Doc version: v0.1.0 | skeleton / REUSE-TM-1 new reusable starter | new reusable file added | Copy/adapt into project-local tools/tampermonkey; not command authority. |
| `planning/documentation/tools/tampermonkey/chat-command-palette.starter.user.js` | reusable starter userscript template | `planning/documentation/tampermonkey-command-projection-workflow.md` | reusable starter implementation template | starter userscript @version 0.1.0; projection workflow Doc version: v0.1.0 | skeleton / REUSE-TM-1 new reusable starter | node syntax check required on package generation | Starter template only; project-local helper owns adapted implementation. |
| `planning/documentation/use-case-map-workflow.md` | reusable UCM maintenance workflow and setup-time example-fit pass | `planning/documentation/examples/README.md`; `planning/documentation/field-kits/root-use-case-map-field-kit.md`; `planning/documentation/example-coverage-workflow.md`; `planning/planning-use-case-map.md` | workflow/process | workflow Doc version: v0.1.0; examples README Doc version: v0.2.0; field kit/example coverage Doc version: v0.1.0; UCM Doc version: v1.0.0 | skeleton / CASCADE-ROUTE-1B limited file-level sync | setup-time only for broad example-fit pass | Maintenance should not reread every example by default. |
...
| `planning/workstreams/slice-test-source-refactor-goal-map.md` | active slice/testing source-refactor living Goal Map | `planning/goal-map-principles-workflow-template.md`; `planning/planning-use-case-map.md`; `planning/source-cascade-sync-workflow.md`; `planning/slices/slice-source-sync-register.md`; `planning/testing/testing-source-sync-register.md`; versioned slice/testing owner docs | workstream state + audit/refactor navigation | map Doc version: v0.2.0; source-cascade workflow Doc version: v1.0.0; slice/testing registers Doc version: v0.2.0; root register Doc version: v2.0.0 | skeleton / SLICE-REF-GM-0-AUD1 audit findings recorded | no concrete slice/test coverage claim | Tracks current slice/test refactor goal, inventory/gap stats, candidate audit cards and pilot decision before first per-slice refactor. |
...
| `planning/workstreams/slice-test-source-refactor-goal-map.md` | slice/testing registers and versioned owner docs | Local workstream map now has AUD1 inventory/gap findings and candidate cards. | audit findings recorded / concrete slice coverage unsynchronized | Choose pilot only after source-chain preflight/decision is recorded. |
...
| `planning/planning-use-case-map.md` / `planning/workflow-activation-map.md` / `planning/planning-doc-responsibility-map.md` | route/read/navigation links | CASCADE-ROUTE-1B clarified that these links are routing/ownership/activation relationships by default, not broad source/version dependency edges. | current cleanup / limited file-level sync | Keep classifying uncertain relations through `сорс-импакт`; do not reintroduce everything-depends-on-everything rows. |
...
- SLICE-REF-GM-0 created the slice/test source-refactor living Goal Map, bumped this register to Doc version: v1.8.0 and kept concrete slice/test coverage unsynchronized.

- SLICE-REF-GM-0-AUD1 recorded inventory/gap findings and pilot candidate cards in the slice/test refactor Goal Map, bumped this register to Doc version: v1.9.0 and kept concrete slice/test coverage unsynchronized.

- CASCADE-CMD-PREFLIGHT-0 added command-aware Workflow Preflight hardening, updated Tampermonkey command-body guardrails, clarified `давай архив` as full replacement archive mode, bumped helper projection docs/userscript versions, parked the larger action-type framework as deferred, and bumped this register to Doc version: v2.0.0 while preserving slice/testing and parallel-work rows.

### CMD-TM-EN-NAMES-1

```text
Source Delta:
  - Updated UCM, Tampermonkey projection docs, helper README, userscript and command-system Goal Map for neutral English command/use-case names.
  - Updated tracked versions for UCM v1.1.0, projection plan v0.4.0, helper README v0.4.0, userscript @version 0.4.0 and command-system Goal Map v0.3.0.
  - English names are display labels only and do not create a new command source-of-truth layer.
```


## Source Delta — REUSE-TM-1

```text
Changed files:
  - planning/documentation/README.md @ reusable layer navigation / read order
  - planning/documentation/documentation-responsibility-map.md @ documentation-layer placement rules
  - planning/documentation/PORTABLE-STARTER-KIT.md @ portable copy/adaptation guidance
  - planning/documentation/field-kits/root-use-case-map-field-kit.md @ root UCM seed rows and command bootstrap rules
  - planning/documentation/command-creation-workflow.md @ command creation process and reusable Tampermonkey decision gate
  - planning/documentation/tampermonkey-command-projection-workflow.md @ new reusable projection workflow
  - planning/documentation/tools/tampermonkey/README.md @ new reusable tool starter notes
  - planning/documentation/tools/tampermonkey/chat-command-palette.starter.user.js @ new reusable helper starter template
  - planning/workstreams/tampermonkey-command-projection-plan.md @ Enman-specific application plan, now linked to reusable projection workflow
  - tools/tampermonkey/README.md @ Enman-specific implementation entrypoint, now linked to reusable projection workflow

Reason:
  The copied reusable docs layer must contain the general Tampermonkey projection rules and reusable tool starter notes. Enman-specific workstream and tools files remain local application/implementation owners only.

Boundary:
  This does not change the project-local userscript command list. It moves reusable ownership and starter guidance into planning/documentation/.
```


## Source Delta — ROOT-LEGACY-CLEAN-STEP2

```text
Changed files:
  - planning/README.md @ active navigation after legacy deletion
  - planning/documentation/README.md @ reusable layer navigation after documentation-legacy removal
  - planning/documentation/PORTABLE-STARTER-KIT.md @ copy guidance after legacy folder removal
  - planning/planning-doc-responsibility-map.md @ root responsibility map after legacy route removal / Doc version v0.6.0
  - planning/agent-roles-and-required-actions.md @ stale read path cleanup
  - planning/root-source-sync-register.md @ root inventory collapse after deleted legacy/current-like files
  - planning/documentation-action-log.md @ append-only cleanup record

Reason:
  Legacy/current-like root files and documentation-legacy were removed, but active docs still referenced them as current or pending inventory. New chats should not route to removed files.

Boundary:
  This cleanup does not change reusable Tampermonkey projection rules, project-local userscript command profiles or slice/testing source refactor state. It only syncs active documentation after deletion.
```
