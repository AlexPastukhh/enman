...
| `параллельный агент`, `работай параллельно`, `parallel workspace`, `parallel work`, `создай parallel workspace` | Use or plan a staging-only parallel workspace for one agent/workstream when shared canonical docs may conflict with other chats. | Ask for scope if no active workstream or target files are clear. | Targeted/full by workspace scope. | Conversation plus current canonical target files and `planning/documentation/parallel-work/` workflow/template docs. | Parallel workspace plan or package when separately requested; does not edit shared canonical docs directly. |
