# pmr-source-usage-action-log-boundary-v2.ps1
# ASCII-safe targeted local edit for planning/planning-maintenance-register.md.
# Avoids literal Unicode punctuation in the script source for Windows PowerShell compatibility.
#
# Run from repository root:
#   powershell -ExecutionPolicy Bypass -File .\pmr-source-usage-action-log-boundary-v2.ps1
...
$newHeading = "### PMR-002 $dash Create source usage cascade workflow after pilot"
...
$newPmr002 = $newHeading + "`r`n`r`n" + $newPmr002Body

# Match the whole PMR-002 section regardless of heading wording/dash encoding, ending before PMR-003.
$pmr002Pattern = '(?s)### PMR-002[^\r\n]*\r?\n\s+```text.*?```\s+(?=### PMR-003)'

if ($raw -match $pmr002Pattern) {
  $raw = [regex]::Replace($raw, $pmr002Pattern, [System.Text.RegularExpressions.MatchEvaluator]{ param($m) $newPmr002 }, 1)
  Write-Host "Replaced PMR-002 block."
} else {
  throw "Could not find PMR-002 block."
}
...
$requiredMarkers = @(
  "This register is not the documentation action log.",
  "PMR-002",
  "Create source usage cascade workflow after pilot",
  "Area: source usage / cascade / layer encapsulation",
  "Governance plan and pilot skeleton exist. Full workflow is still not created.",
  "Do not use PMR as the general log of all documentation actions."
)
...
if ($raw -like "*Create source/version cascade sync workflow after pilot*") {
  throw "Validation failed. Old PMR-002 heading text is still present."
}

# 5) Write file with UTF-8 BOM for Windows PowerShell friendliness.
$utf8Bom = New-Object System.Text.UTF8Encoding($true)
[System.IO.File]::WriteAllText((Resolve-Path $path), $raw, $utf8Bom)
