...
Ниже полный обновлённый драфт **v2**. В него добавлены: API как сценарный контракт, command как intention/action, запрет передачи нового статуса с frontend, route id vs body, screen-specific read DTO, command result DTO, stale DTO, повторные команды, concurrency/business-state conflicts, сокрытие infrastructure details, stable codes vs labels, pagination/filter/sort, API contract ownership, controlled API changes и API-антипаттерны. Основа обновления — присланный материал по усилению 2.5.1. 
