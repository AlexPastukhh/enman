...
| `USE-CASE-MAP-CREATION-UPDATE-v1` | workflow/template example | `planning/documentation/use-case-map-workflow.md` and `planning/documentation/USE-CASE-MAP-TEMPLATE.md` | reusable use-case-map creation/update | deferred | _not created yet_ | Add after the workflow/template is used on a future concrete use-case map; should show route rows, expected-output ownership and workflow/template links. |
